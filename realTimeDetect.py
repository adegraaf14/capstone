        ...
        for contour in contours:
            ...
            if cv.contourArea(contour) > 10: #ignoring small contours
                ...
                if is_closed:
                    ...
                    cv.drawContours(temp_mask, [contour], -1, fill_color, thickness=cv.FILLED)
        
        filled_only = cv.subtract(temp_mask, mask)
        combined_mask = cv.add(mask, filled_only)
        cv.imshow("Filled only", resize_image(filled_only))
            
 
        
    frame = cv.circle(frame, maxLoc, 5, color, -1)
    
    #img is the frame with the line drawn over it
