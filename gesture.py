...
def calculate_confidence_scores(features):
    ...
    # Calculate triangle confidence - MODIFIED FOR STRICTER DETECTION
    # Ideal triangle has exactly 3 direction changes, compactness ~1.3, is_triangle_like=True
    triangle_direction = max(0, 1 - abs(features['direction_changes'] - 3) / 1.5)  # Stricter requirement for exactly 3 changes
    triangle_compactness = max(0, 1 - abs(features['compactness'] - 1.3) / 0.5)  # Stricter compactness requirement
    triangle_shape = 1.0 if features['is_triangle_like'] else 0.1  # Much stronger emphasis on triangle hull shape
    
    # Apply a stricter weighting that requires more triangle-like features
    confidence['triangle'] = int((triangle_direction * 0.3 + triangle_compactness * 0.3 + triangle_shape * 0.4) * 100)
